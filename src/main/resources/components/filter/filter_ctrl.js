...
angular.module('filter', ['ui.bootstrap'])
    .controller('filterCtrl', ['$scope','UrlService', function ($scope, UrlService) {
        $scope.filterList=[];
    ...
    }]).controller('filterIPCtrl',function($scope,UrlService){
		$scope.type="";
		...
		function setValues(fltr) {			
			...
			angular.forEach($scope.filters, function(filter,index){
				if(filter.value==fltr.name){
					...
					$scope.operators=UrlService.getOperators(filter.type);	
					console.log(JSON.stringify($scope.operators));
					$scope.valueList = UrlService.getList(fltr.name);
				}
			});				
		}
